...
class M_info_mempelai extends CI_Model{
    ...
    public function cek_cerita(){
        $ada = 'ada';
        $tidak = 'tidak_ada';
        $id_acara = $_SESSION['id_acara'];
        $sql = "SELECT * FROM cerita_cinta WHERE id_acara ='".$id_acara."'";
        $query = $this->db->query($sql, array($id_acara));
        if ($query->num_rows() > 0) {
            // Data ditemukan
            return $ada;
    ...
    public function cek_galeri(){
        $ada = 'ada';
        $tidak = 'tidak_ada';
        $id_acara = $_SESSION['id_acara'];
        $sql = "SELECT * FROM galeri_foto WHERE id_acara ='".$id_acara."'";
        $query = $this->db->query($sql, array($id_acara));
        if ($query->num_rows() > 0) {
            // Data ditemukan
            return $ada;
   ...
   public function pakai_template($template,$id_acara)
   {
    ...
    public function tambahEvent($pilih_opsi_rb,$music_filename)
    {
        ...
        $music_filename = $music_filename;
            ...
            $this->load->library('upload', $config_gambar);
    ...
    public function tambah_cerita_cinta()
    {
        $id_acara_resepsi = $_SESSION['id_acara'];
        //Multiple input
    ...
    public function galeri_foto(){
        ...
        $id_acara = $_SESSION['id_acara'];
        $query_galeri = $this->db->query("SELECT * FROM galeri_foto where id_acara='$id_acara'");
        $hasil_galeri = $query_galeri->result_array();
        return $hasil_galeri;
    }
    public function cerita_cinta(){

        $id_acara = $_SESSION['id_acara'];
        $query_cerita_cinta = $this->db->query("SELECT * FROM cerita_cinta where id_acara='$id_acara'");
        $hasil_cerita_cinta = $query_cerita_cinta->result_array();
        return $hasil_cerita_cinta;
    }
    ...
    public function updateEvent($pilih_opsi_rb)
    {
        $id_acara_resepsi = $_SESSION['id_acara'];
        //get id_banner 
        $query_cekid = $this->db->query("SELECT * FROM undangan WHERE id_acara_resepsi='$id_acara_resepsi';");
        $code_idbanner = $query_cekid->row()->id_banner;
        ...
        $this->load->library('upload');
            ...
            if ($this->upload->do_upload('musik_cover')) {
                ...
                $music_filename = $this->db->query("SELECT * FROM undangan WHERE id_acara_resepsi='$id_acara_resepsi';")->row()->musik;
            }
            ...
            if ($this->upload->do_upload('cover_img')){
                ...
                $gambar_filename = $this->db->query("SELECT * FROM undangan WHERE id_acara_resepsi='$id_acara_resepsi';")->row()->gambar_cover;
            }
        ...
        $this->db->where('id_acara_resepsi', $id_resepsi);
        $this->db->update('undangan', $data);
    }


    public function update_InfoMempelai()
    {
        $id_acara_resepsi = $_SESSION['id_acara'];
        ...
        if ($this->upload->do_upload('foto_pria')){
            ...
            $gambar_pria_filename = $this->db->query("SELECT * FROM info_pria WHERE id_acara_resepsi='$id_acara_resepsi';")->row()->foto;
        }
         ...
         if ($this->upload->do_upload('foto_wanita')){
            ...
            $gambar_wanita_filename = $this->db->query("SELECT * FROM info_wanita WHERE id_acara_resepsi='$id_acara_resepsi';")->row()->foto;
         }
        ...
        $this->db->where('id_acara_resepsi', $id_acara_resepsi);
        $this->db->update('info_pria', $data_pria);

        $this->db->where('id_acara_resepsi', $id_acara_resepsi);
        $this->db->update('info_wanita', $data_wanita);

    }


    
    public function update_galeri_foto($data_lama)
    {
        $this->load->library('upload');
    
        $id_acara_resepsi = $_SESSION['id_acara'];
    
        // Menghapus data galeri foto yang sudah ada
        $this->db->where('id_acara', $id_acara_resepsi);
        $this->db->delete('galeri_foto');
    
        $code_id_galeri = $this->get_IdGaleri();
        // Menyimpan data lama
        foreach ($data_lama as $foto_lama) {
            $insert_data1 = array(
                'id_galeri' => $code_id_galeri,
                'id_acara' => $id_acara_resepsi,
                'foto' => $foto_lama
            ...
            $this->db->insert('galeri_foto', $insert_data1);
        }
        ...
        $data = array(); // Tambahkan inisialisasi variabel $data sebagai array kosong
        ...
        // Tambahkan data baru ke galeri foto
        foreach ($data as $filename) {
            $insert_data = array(
                'id_galeri' => $code_id_galeri,
                'id_acara' => $id_acara_resepsi,
    ...
    public function update_cerita_cinta($data_tanggal, $data_judul, $data_cerita)
    {
        $id_acara_resepsi = $_SESSION['id_acara'];

        // Hapus semua data cerita cinta yang terkait dengan acara
        $this->db->where('id_acara', $id_acara_resepsi);
        $this->db->delete('cerita_cinta');

        // Ambil ID cerita cinta terbaru
        $code_id_cerita = $this->M_info_mempelai->get_Idcerita_cinta();
        ...
        // Proses setiap data dari form
        foreach ($judul as $index => $data_judul) {
            // Buat array dengan data cerita
            $cerita_data = array(
                'id_cerita_cinta' => $code_id_cerita,
                'id_acara' => $id_acara_resepsi,
                'judul_cerita' => $data_judul,
            ...
            // Tambahkan data cerita ke database
            $this->db->insert('cerita_cinta', $cerita_data);

            // Buat ID cerita cinta berikutnya
            $Id_khusus = (int) preg_replace('/\D/', '', $code_id_cerita);
            $id_cerita = "C" . ($Id_khusus + 1);
            $code_id_cerita = $id_cerita;
        }
